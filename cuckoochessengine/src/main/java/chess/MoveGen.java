...
public final class MoveGen {
    ...
    public static final boolean sqAttacked(Position pos, int sq) {
        if (pos.whiteMove) {
            ...
            if ((BitBoard.bishopAttacks(sq, occupied) & (pos.pieceTypeBB[Piece.BBISHOP] | bbQueen))
                    != 0) {
                return true;
            }
            if ((BitBoard.rookAttacks(sq, occupied) & (pos.pieceTypeBB[Piece.BROOK] | bbQueen))
                    != 0) {
                return true;
            }
        } else {
            ...
            if ((BitBoard.bishopAttacks(sq, occupied) & (pos.pieceTypeBB[Piece.WBISHOP] | bbQueen))
                    != 0) {
                return true;
            }
            if ((BitBoard.rookAttacks(sq, occupied) & (pos.pieceTypeBB[Piece.WROOK] | bbQueen))
                    != 0) {
                return true;
            }
        }
    ...
    private final static boolean addMovesByMask(MoveList moveList, Position pos, int sq0,
            long mask) {
        long oKingMask = pos.pieceTypeBB[pos.whiteMove ? Piece.BKING : Piece.WKING];
        if ((mask & oKingMask) != 0) {
            int sq = BitBoard.numberOfTrailingZeros(mask & oKingMask);
    ...
    public final MoveList checkEvasions(Position pos) {
        ...
        if (pos.whiteMove) {
            ...
            long validTargets = 0;
            if ((kingThreats != 0) && ((kingThreats & (kingThreats - 1))
                    == 0)) { // Exactly one attacking piece
    ...
    public final MoveList pseudoLegalCapturesAndChecks(Position pos) {
        ...
        if (pos.whiteMove) {
            ...
            while (squares != 0) {
                int sq = BitBoard.numberOfTrailingZeros(squares);
                long m = (BitBoard.rookAttacks(sq, occupied) | BitBoard.bishopAttacks(sq,
                        occupied));
            ...
            // Normal checks
            m = ((pawns & ~pawnAll) << 8) & ~(pos.whiteBB | pos.blackBB);
            if (addPawnMovesByMask(moveList, pos, m & BitBoard.bPawnAttacks[bKingSq], -8, false)) {
                return moveList;
            }
            m = ((m & BitBoard.maskRow3) << 8) & ~(pos.whiteBB | pos.blackBB);
            ...
            // Normal checks
            m = ((pawns & ~pawnAll) >>> 8) & ~(pos.whiteBB | pos.blackBB);
            if (addPawnMovesByMask(moveList, pos, m & BitBoard.wPawnAttacks[wKingSq], 8, false)) {
                return moveList;
            }
            m = ((m & BitBoard.maskRow6) >>> 8) & ~(pos.whiteBB | pos.blackBB);
