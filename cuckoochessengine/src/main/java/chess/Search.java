...
public class Search {
    ...
    History ht;
    long[] posHashList;
            // List of hashes for previous positions up to the last "zeroing" move.
    ...
    public final static boolean canClaimDrawRep(Position pos, long[] posHashList,
            ...
            for (int depthS = plyScale; ; depthS += plyScale, firstIteration = false) {
                ...
                for (int mi = 0; mi < scMoves.length; mi++) {
                    ...
                    if (firstIteration) {
                        ...
                        beta = (mi == 0) ? Math.min(bestScoreLastIter + aspirationDelta,
                                Search.MATE0) : alpha + 1;
                    }
...
/*                long nodes0 = nodes;
                ...
                System.out.printf("%-6s...\n", TextIO.moveToUCIString(m)); */
                    ...
                    sti.lmr = lmrS;
                    sti.nodeIdx = -1;
                    int score = -negaScout(-beta, -alpha, 1, depthS - lmrS - plyScale, -1,
                            givesCheck);
                    ...
                    if (score >= beta) {
                        ...
                        while (score >= beta) {
                            ...
                            posHashList[posHashListSize++] = pos.zobristHash();
                            pos.makeMove(m, ui);
                            int score2 = -negaScout(-beta, -score, 1, depthS - plyScale, -1,
                                    givesCheck);
                        ...
                        while (score <= alpha) {
                            ...
                            posHashList[posHashListSize++] = pos.zobristHash();
                            pos.makeMove(m, ui);
                            score = -negaScout(-score, -alpha, 1, depthS - plyScale, -1,
                                    givesCheck);
                            nodesThisMove += nodes + qNodes;
                            posHashListSize--;
                            pos.unMakeMove(m, ui);
...
/*                        System.out.printf("%2d %5d %5d %5d %6d %6d ",
    ...
    final public int negaScout(int alpha, int beta, int ply, int depth, int recaptureSquare,
        ...
        if (log != null) {
            ...
            long idx = log.logNodeStart(sti.nodeIdx, sti.currentMove, alpha, beta, ply,
                    depth / plyScale);
        ...
        if (canClaimDraw50(pos)) {
            if (MoveGen.canTakeKing(pos)) {
                int score = MATE0 - ply;
                if (log != null) {
                    log.logNodeEnd(searchTreeInfo[ply].nodeIdx, score, TTEntry.T_EXACT,
                            UNKNOWN_SCORE, hKey);
                }
                return score;
            }
        ...
        if (ent.type != TTEntry.T_EMPTY) {
            ...
            if ((beta == alpha + 1) && ((eDepth >= depth) || (eDepth >= plyToMate * plyScale))) {
                if ((ent.type == TTEntry.T_EXACT) ||
                    ...
                    if (log != null) {
                        log.logNodeEnd(searchTreeInfo[ply].nodeIdx, score, ent.type,
                                evalScore, hKey);
                    }
                    return score;
                }
        ...
        if (!inCheck && (depth < 5 * plyScale) && (posExtend == 0) &&
            ...
            if (mtrlOk) {
                ...
                if (evalScore - margin >= beta) {
                    ...
                    if (log != null) {
                        log.logNodeEnd(sti.nodeIdx, evalScore - margin, TTEntry.T_GE,
                                evalScore, hKey);
                    }
        ...
        if ((depth >= 3 * plyScale) && !inCheck && sti.allowNullMove &&
            ...
            if (MoveGen.canTakeKing(pos)) {
                int score = MATE0 - ply;
                if (log != null) {
                    log.logNodeEnd(sti.nodeIdx, score, TTEntry.T_EXACT, evalScore,
                            hKey);
                }
                return score;
            }
            ...
            if (nullOk) {
                ...
                if (score >= beta) {
                    ...
                    tt.insert(hKey, emptyMove, TTEntry.T_GE, ply, depth, evalScore);
                    if (log != null) {
                        log.logNodeEnd(sti.nodeIdx, score, TTEntry.T_GE, evalScore,
                                hKey);
                    }
                    return score;
                } else {
                    if ((searchTreeInfo[ply - 1].lmr > 0) && (depth < 5 * plyScale)) {
                        ...
                        if (relatedMoves(m1, m2)) {
                            ...
                            if (log != null) {
                                log.logNodeEnd(sti.nodeIdx, alpha, TTEntry.T_LE,
                                        evalScore, hKey);
                            }
                            return alpha;
                        }
        ...
        for (int mi = 0; mi < moves.size; mi++) {
            ...
            if (doFutility) {
                ...
                if (posExtend == 0) {
                    ...
                    if ((moveExtend < plyScale) && isCapture && (pos.wMtrlPawns + pos.bMtrlPawns
                            > pV)) {
                        ...
                        if (pos.whiteMove) {
                            ...
                            if ((pos.bMtrl == pos.bMtrlPawns) && (pos.wMtrl - pos.wMtrlPawns
                                    == capVal)) {
                                moveExtend = plyScale;
                            }
                        }
...
/*              long nodes0 = nodes;
                ...
                if (((lmr > 0) && (score > alpha)) ||
                        ((score > alpha) && (score < beta) && (b != beta) && (score
                                != illegalScore))) {
                    sti.lmr = 0;
                    newDepth += lmr;
...
/*              if (ply <= 3) {
        ...
        if (bestMove >= 0) {
            ...
            if (log != null) {
                log.logNodeEnd(sti.nodeIdx, bestScore, TTEntry.T_EXACT, evalScore,
                        hKey);
            }
        } else {
    ...
    private final boolean weakPlaySkipMove(Position pos, Move m, int ply) {
        ...
        double effPly = ply * Evaluate.interpolate(pos.wMtrl + pos.bMtrl, 0, 30, Evaluate.qV * 4,
                100) * 1e-2;
        ...
        while (true) {
            ...
            if (white) {
                ...
                if (atk != 0) {
                    ...
                    atk = BitBoard.knightAttacks[square] & pos.pieceTypeBB[Piece.WKNIGHT]
                            & occupied;
                    if (atk != 0) {
                            ...
                            if (atk != 0) {
                                ...
                                if (atk != 0) {
                                    ...
                                    atk = BitBoard.kingAttacks[square]
                                            & pos.pieceTypeBB[Piece.WKING] & occupied;
                                    if (atk != 0) {
                                        bestValue = kV;
                                    } else {
                                        break;
                ...
                if (atk != 0) {
                    ...
                    atk = BitBoard.knightAttacks[square] & pos.pieceTypeBB[Piece.BKNIGHT]
                            & occupied;
                    if (atk != 0) {
                        bestValue = Evaluate.nV;
                    } else {
                        ...
                        if (atk != 0) {
                            ...
                            if (atk != 0) {
                                ...
                                if (atk != 0) {
                                    ...
                                    atk = BitBoard.kingAttacks[square]
                                            & pos.pieceTypeBB[Piece.BKING] & occupied;
                                    if (atk != 0) {
                                        bestValue = kV;
                                    } else {
                                        break;
