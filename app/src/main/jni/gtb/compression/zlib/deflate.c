...
int ZEXPORT deflateInit2_(z_streamp strm, int level, int method, int windowBits, int memLevel,
    ...
    s->pending_buf_size =
            (ulg) s->lit_bufsize * (ulg) (sizeof(ush) + 2L); /*MAB casts (ulg)(sizeof... */
...
/* ========================================================================= */
int ZEXPORT deflateTune(z_streamp strm, int good_length, int max_lazy, int nice_length,
                        int max_chain) {
    deflate_state *s;
...
#pragma warning(default:4127)
        ...
        n = (unsigned) read_buf(s->strm, s->window + s->strstart + s->lookahead,
                                more); /*MAB casts */
        s->lookahead += n;
    ...
    for (; ;) {
        ...
        if (s->match_length >= MIN_MATCH) {
            check_match(s, s->strstart, s->match_start,
                        (int) s->match_length); /*MAB cast with int */
    ...
    for (; ;) {
        ...
        if (s->prev_length >= MIN_MATCH && s->match_length <= s->prev_length) {
            ...
            check_match(s, s->strstart - 1, s->prev_match,
                        (int) s->prev_length); /*MAB cast with int */
