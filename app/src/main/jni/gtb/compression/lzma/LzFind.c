...
static UInt32 *Hc_GetMatchesSpec(UInt32 lenLimit, UInt32 curMatch, UInt32 pos, const Byte *cur,
                                 CLzRef *son,
...
static void SkipMatchesSpec(UInt32 lenLimit, UInt32 curMatch, UInt32 pos, const Byte *cur,
                            CLzRef *son,
                            UInt32 _cyclicBufferPos, UInt32 _cyclicBufferSize, UInt32 cutValue) {
    CLzRef *ptr0 = son + (_cyclicBufferPos << 1) + 1;
    CLzRef *ptr1 = son + (_cyclicBufferPos << 1);
...
static UInt32 Bt3_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances) {
    ...
    if (delta2 < p->cyclicBufferSize && *(cur - delta2) == *cur) {
        for (; maxLen != lenLimit; maxLen++)
            if (cur[(ptrdiff_t) maxLen - (ptrdiff_t) delta2] !=
                cur[maxLen]) /*MAB second casts (ptrdiff_t)*/
                break;
        distances[0] = maxLen;
...
static UInt32 Bt4_MatchFinder_GetMatches(CMatchFinder *p, UInt32 *distances) {
    ...
    if (offset != 0) {
        for (; maxLen != lenLimit; maxLen++)
            if (cur[(ptrdiff_t) maxLen - (ptrdiff_t) delta2] !=
                cur[maxLen]) /*MAB casts second (ptrdiff_t)*/
                break;
