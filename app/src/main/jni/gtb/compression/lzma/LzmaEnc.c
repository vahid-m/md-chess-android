...
void LzmaEncProps_Normalize(CLzmaEncProps *p) {
    ...
    p->level = level;
    if (p->dictSize == 0)
        p->dictSize = (UInt32) ((level <= 5 ? (1 << (level * 2 + 14)) : (level == 6 ? (1 << 25) : (
                1 << 26)))); /*MAB casts UInt32 */
    if (p->lc < 0) p->lc = 3;
    if (p->lp < 0) p->lp = 0;
    if (p->pb < 0) p->pb = 2;
    ...
    if (p->mc == 0)
        p->mc = (UInt32) ((16 + (p->fb >> 1)) >> (p->btMode ? 0 : 1)); /*MAB casts UInt32 */
    if (p->numThreads < 0)
        p->numThreads =
#ifdef COMPRESS_MF_MT
                ((p->btMode && p->algo) ? 2 : 1);
...
static void LzmaEnc_InitPriceTables(UInt32 *ProbPrices) {
    UInt32 i;
    for (i = (1 << kNumMoveReducingBits) / 2;
         i < kBitModelTotal; i += (1 << kNumMoveReducingBits)) {
        ...
        ProbPrices[i >> kNumMoveReducingBits] = (/*MAB: cast to silence*/
                (UInt32) (kNumBitModelTotalBits << kCyclesBits) -
...
static UInt32 LitEnc_GetPriceMatched(const CLzmaProb *probs, UInt32 symbol, UInt32 matchByte,
                                     UInt32 *ProbPrices) {
    UInt32 price = 0;
    UInt32 offs = 0x100;
    symbol |= 0x100;
...
static UInt32 RcTree_GetPrice(const CLzmaProb *probs, int numBitLevels, UInt32 symbol,
                              UInt32 *ProbPrices) {
    UInt32 price = 0;
...
static UInt32 RcTree_ReverseGetPrice(const CLzmaProb *probs, int numBitLevels, UInt32 symbol,
                                     UInt32 *ProbPrices) {
    UInt32 price = 0;
    UInt32 m = 1;
    int i;
...
static void LenEnc_SetPrices(CLenEnc *p, UInt32 posState, UInt32 numSymbols, UInt32 *prices,
    ...
    for (i = 0; i < kLenNumLowSymbols; i++) {
        if (i >= numSymbols)
            return;
        prices[i] = a0 + RcTree_GetPrice(p->low + (posState << kLenNumLowBits), kLenNumLowBits, i,
                                         ProbPrices);
    }
...
static void MY_FAST_CALL LenPriceEnc_UpdateTable(CLenPriceEnc *p, UInt32 posState,
                                                 UInt32 *ProbPrices) {
...
static void LenEnc_Encode2(CLenPriceEnc *p, CRangeEnc *rc, UInt32 symbol, UInt32 posState,
    ...
    if (updatePrice) if (--p->counters[posState] == 0)
        LenPriceEnc_UpdateTable(p, posState, ProbPrices);
}
    ...
    if (len <= mainLen) {
        ...
        for (; ; len++) {
            ...
            UInt32 curAndLenPrice =
                    normalMatchPrice + p->lenEnc.prices[posState_][len - LZMA_MATCH_LEN_MIN];
    ...
    for (; ;) {
        ...
        if (newLen >= startLen) {
            ...
            for (lenTest = /*2*/ startLen; ; lenTest++) {
                ...
                UInt32 curAndLenPrice__ = normalMatchPrice__ +
                ...
                if (curBack < kNumFullDistances)
                    ...
                    curAndLenPrice__ += p->posSlotPrices[lenToPosState][posSlot] +
...
static void WriteEndMarker(CLzmaEnc *p, UInt32 posState) {
    ...
    RcTree_Encode(&p->rc, p->posSlotEncoder[GetLenToPosState(len)], kNumPosSlotBits,
                  (1 << kNumPosSlotBits) - 1);
    RangeEnc_EncodeDirectBits(&p->rc, (((UInt32) 1 << 30) - 1) >> kNumAlignBits,
                              30 - kNumAlignBits);
...
static void FillAlignPrices(CLzmaEnc *p) {
    ...
    for (i = 0; i < kAlignTableSize; i++)
        p->alignPrices[i] = RcTree_ReverseGetPrice(p->posAlignEncoder, kNumAlignBits, i,
                                                   p->ProbPrices);
    p->alignPriceCount = 0;
}
...
static void FillDistancesPrices(CLzmaEnc *p) {
    ...
    for (j = kStartPosModelIndex; j < kNumFullDistances; j++) {
        ...
        tempPrices[j] = RcTree_ReverseGetPrice(p->posEncoders + base - posSlot - 1,
                                               (int) footerBits, j - base,
    ...
    for (lenToPosState = 0; lenToPosState < kNumLenToPosStates; lenToPosState++) {
        ...
        for (posSlot = 0; posSlot < p->distTableSize; posSlot++)
            posSlotPrices[posSlot] = RcTree_GetPrice(encoder, kNumPosSlotBits, posSlot,
                                                     p->ProbPrices);
...
static SRes LzmaEnc_CodeOneBlock(CLzmaEnc *p, Bool useLimits, UInt32 maxPackSize,
    ...
    if (p->nowPos64 == 0) {
        ...
        curByte = p->matchFinder.GetIndexByte(p->matchFinderObj,
                                              (Int32) (0 - p->additionalOffset)); /*MAB casts */
            ...
            if (len == 1 && pos == (UInt32) -1) {
                ...
                if (pos < LZMA_NUM_REPS) {
                    ...
                    if (len == 1)
                        ...
                        LenEnc_Encode2(&p->repLenEnc, &p->rc, len - LZMA_MATCH_LEN_MIN, posState,
                    ...
                    if (posSlot >= kStartPosModelIndex) {
                        ...
                        if (posSlot < kEndPosModelIndex)
                            RcTree_ReverseEncode(&p->rc, p->posEncoders + base - posSlot - 1,
                                                 (int) footerBits, posReduced); /*MAB casts */
                        else {
                            RangeEnc_EncodeDirectBits(&p->rc, posReduced >> kNumAlignBits,
                                                      (int) footerBits -
                                                      kNumAlignBits); /*MAB casts */
...
static SRes LzmaEnc_Alloc(CLzmaEnc *p, UInt32 keepWindowSize, ISzAlloc *alloc, ISzAlloc *allocBig) {
...

    {
        ...
        if (p->litProbs == 0 || p->saveState.litProbs == 0 || p->lclp != lclp) {
            LzmaEnc_FreeLits(p, alloc);
            p->litProbs = (CLzmaProb *) alloc->Alloc(alloc, (0x300u << lclp) *
                                                            sizeof(CLzmaProb)); /*MAB 0x300 vs 0x300u*/
            p->saveState.litProbs = (CLzmaProb *) alloc->Alloc(alloc, (0x300u << lclp) *
                                                                      sizeof(CLzmaProb)); /*MAB 0x300 vs 0x300u*/
    ...
    for (; ;) {
        ...
        if (progress != 0) {
            res = progress->Progress(progress, p->nowPos64,
                                     (UInt64) RangeEnc_GetProcessed(&p->rc)); /*MAB casts */
            if (res != SZ_OK) {
