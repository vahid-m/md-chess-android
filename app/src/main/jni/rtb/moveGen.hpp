...
public:
    ...
    static bool sqAttacked(const Position &pos, int sq, U64 occupied) {
        ...
        if ((BitBoard::bishopAttacks(sq, occupied) &
             (pos.pieceTypeBB(OtherColor::BISHOP) | bbQueen)) != 0)
            return true;
        if ((BitBoard::rookAttacks(sq, occupied) & (pos.pieceTypeBB(OtherColor::ROOK) | bbQueen)) !=
            0)
            return true;
        return false;
    }
